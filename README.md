...
텍스트 옵션만 또는 참조 사진만 입력하면 초기 방식처럼 기본 편집 마스크와 프롬프트(사진 입력 시 참조 사진 포함)로 각 미리보기를 FLUX에서 한 번씩 생성합니다.
영역별 재생성과 수치 기반 염색은 적용하지 않습니다. 추천색마다 헤어 형태가 조금 달라질 수 있습니다.

참조 사진과 텍스트 옵션을 함께 입력한 경우 길이·웨이브는 옆·아래 머리 영역에서 보정하고, 선택한 앞머리를 마지막에 별도로 보정합니다.
부분 보정에는 참조 사진을 다시 넣지 않으며, 편집 영역 밖 픽셀은 복원합니다.
옵션에 따라 요청 스타일 생성에 FLUX 추론이 최대 3회 필요합니다.
두 입력을 함께 사용하는 경로의 추천색은 최종 요청 이미지의 머리색만 변경합니다.
